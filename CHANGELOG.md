...
- **DSC extension wired end-to-end** (Layer 4 of the 5-layer auto-heal model). The v1.1.0 extension only set `ConfigurationModeFrequencyMins` with no `configuration` block - it was effectively a no-op. The new extension consumes a release-asset zip published by a separate DSC packaging repository. New inputs: `dsc_enabled` (bool, default `true`), `dsc_config_url`, `dsc_config_sas_token` (sensitive), `dsc_configuration_script` (default `"RunnerSupervisor.ps1"`), `dsc_configuration_function` (default `"RunnerSupervisor"`), `dsc_configuration_arguments` (map). Consumers fetch the zip via `data.http`, push to a private blob, and pass URL + SAS - see the DSC packaging repository's consuming guide. The module-internal `runner-dsc-config.ps1` stub has been removed (was duplicating the DSC configuration without the watchdog + supervisor enforcement).
- **Canonical tag taxonomy**: new `canonical_tags` input (object with optional `owner`, `workload`, `pool`, `trust`, `cost_center`). The module auto-injects `Module`, `ModuleVersion`, `OS = "windows"` and merges everything in `var.tags` on top. Standardizes tag keys across the Windows and Linux runner modules so resources are uniformly discoverable.
- **Flexible orchestration mode**: new `orchestration_mode` input (default `"Uniform"` for v1.1.0 back-compat). Set to `"Flexible"` for VMSS-VMs orchestration (required by public-facing pools). The module now omits `upgradePolicy` for Flexible (rejected by ARM otherwise). Immutable - changing this value triggers destroy/recreate of the VMSS. Closes #19.
...
- Documented the Windows VMSS hardening checks for Key Vault secret expiry, DSC storage blob RBAC, and Windows DCR schema risks.
